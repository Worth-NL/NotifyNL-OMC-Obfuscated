...
namespace EventsHandler.UnitTests.Services.Responding
{
    ...
    public sealed class OmcResponderTests
    {
            ...
            (string Id, ProcessingResult Result, ProcessingStatus Status, HttpStatusCode Code, int ObjResultCode, string ObjResultName, string Description, string Content)> GetTestResults()
        {
            ...
            yield return ("#4", ProcessingResult.NotPossible(TestDescription, TestJson, s_errorDetails), ProcessingStatus.NotPossible, HttpStatusCode.UnprocessableEntity, 206, nameof(SimpleDetails), $"{Resources.Operation_ERROR_Deserialization_Failure} | {TestDescription} | Notification: {TestJson}.", $"{{ Message = {TestMessage} }}");
