...
namespace EventsHandler.Tests.Unit.Services.Responding
{
    ...
    public sealed class OmcResponderTests
    {
        ...
        private static readonly ErrorDetails s_httpErrorDetails = new(ApiResources.HttpRequest_ERROR_NoCase, TestCases, s_testReasons);
            ...
            (string Id, ProcessingResult Result, ProcessingStatus Status, HttpStatusCode Code, int ObjResultCode, string ObjResultName, string Description, string Content)> GetTestResults()
        {
            ...
            yield return ("#4", ProcessingResult.NotPossible(TestDescription, TestJson, s_errorDetails), ProcessingStatus.NotPossible, HttpStatusCode.UnprocessableEntity, 206, nameof(ErrorDetails), $"{ApiResources.Operation_ERROR_Deserialization_Failure} | {TestDescription} | Notification: {TestJson}.", $"{{ Message = {TestMessage}, Cases = {TestCases}, Reasons = {s_testReasons} }}");
            yield return ("#5", ProcessingResult.Failure(TestDescription, TestJson, s_httpErrorDetails), ProcessingStatus.Failure, HttpStatusCode.BadRequest, 400, nameof(SimpleDetails), $"{ApiResources.Operation_ERROR_HttpRequest_Failure} | {TestDescription} | Notification: {TestJson}.", $"{{ Message = {ApiResources.HttpRequest_ERROR_NoCase} }}");
